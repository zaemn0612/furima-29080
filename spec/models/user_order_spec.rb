...
RSpec.describe UserOrder, type: :model do
  before do
    ...
    @user_order = FactoryBot.build(:user_order, user_id:@user_id, item_id:@item_id) #user_idはキーで空の状態なので、バリューである@user.idを記載することで、キーに入る。配列に入れる際は<<を使用し、ハッシュの際は:でやると導入される。実際の数値は入れない（実際登録させたものでないとテストはできないため）
